// SERVER ONLY — domain logic + store access behind the `/api/v1` routes.
// Ported from the former client-side mock (`src/dashboard/api/client.ts`);
// behaviour is identical, minus the artificial latency. Functions are
// synchronous because the store is in-memory; the route handlers and the
// HTTP client keep the async surface.
import type {
  Agent,
  AgentRun,
  AgentToggle,
...
// ── Errors (mapped to HTTP status codes in ./http) ──────────────────

/** 404 — the requested entity does not exist. */
export class NotFoundError extends Error {}

/** 400/409 — the request is invalid for the current state. */
...
function assertFound<T>(value: T | undefined, what: string): T {
  if (!value) throw new NotFoundError(`${what} not found`);
  return value;
}
...
export function runFullCycle(): RunCycleResult {
  const s = getState();
...
export function listLeads(filters?: LeadFilters): Lead[] {
...
export function createLead(input: CreateLeadInput): Lead {
  if (!LEAD_STATUSES.includes(input.status)) throw new ValidationError(`Invalid lead status: ${input.status}`);
  const now = nowIso();
...
export function updateLead(id: string, patch: Partial<Omit<Lead, 'id' | 'createdAt'>>): Lead {
  if (patch.status && !LEAD_STATUSES.includes(patch.status)) {
    throw new ValidationError(`Invalid lead status: ${patch.status}`);
  }
...
export function deleteLead(id: string): void {
  ...
  if (idx === -1) throw new NotFoundError(`Lead ${id} not found`);
...
export function listApprovals(filters?: ApprovalFilters): ApprovalItem[] {
...
export function updateApproval(id: string, edits: ApprovalEdits): ApprovalItem {
  const item = assertFound(getState().approvals.find((a) => a.id === id), `Approval ${id}`);
  if (item.status !== 'pending_approval') throw new ValidationError(`Approval ${id} already decided`);
...
export function listContent(filters?: ContentFilters): ContentCalendar[] {
...
export function createPost(input: CreatePostInput): ContentCalendar {
...
export function updatePost(id: string, patch: Partial<Omit<ContentCalendar, 'id' | 'createdAt'>>): ContentCalendar {
  if (patch.status && !PIPELINE_STAGES.includes(patch.status)) {
    throw new ValidationError(`Invalid pipeline stage: ${patch.status}`);
  }
...
export function updatePostStatus(id: string, status: PipelineStage): ContentCalendar {
...
export function listProductIdeas(): ProductIdea[] {
...
export function createProductIdea(input: CreateProductIdeaInput): ProductIdea {
...
export function updateProductIdea(id: string, patch: Partial<Omit<ProductIdea, 'id' | 'createdAt'>>): ProductIdea {
  if (patch.stage && !PIPELINE_STAGES.includes(patch.stage)) {
    throw new ValidationError(`Invalid pipeline stage: ${patch.stage}`);
  }
...
export function moveProductIdea(id: string, stage: PipelineStage): ProductIdea {
  return updateProductIdea(id, { stage });
}

export function deleteProductIdea(id: string): void {
  const s = getState();
  const idx = s.productIdeas.findIndex((p) => p.id === id);
  if (idx === -1) throw new NotFoundError(`Product idea ${id} not found`);
...
export function listMessages(filters?: MessageFilters): ChatMessage[] {
 ...
 * deterministic templated reply from the target agent.
 */
export function sendMessage(content: string, targetAgent?: string): { userMessage: ChatMessage; reply: ChatMessage | null } {
  const s = getState();
...
export function getChatState(): { paused: boolean } {
  return { paused: getState().chatPaused };
}

export function setChatPaused(paused: boolean): { paused: boolean } {
...
export function listLogs(filters?: LogFilters): LogEntry[] {
...
export function listBrandDocs(): BrandDoc[] {
  return clone(getState().brandDocs);
}

export function saveBrandDoc(id: string, content: string): BrandDoc {
  const s = getState();
...
export function listRevisions(docId?: string): DocumentRevision[] {
...
export function restoreRevision(revisionId: string): BrandDoc {
  const s = getState();
...
export function getBrandMemory(): BrandMemoryEntry[] {
  return clone(getState().brandMemory);
}

export function saveBrandMemory(
  entries: Array<Omit<BrandMemoryEntry, 'lastUpdated'> & { lastUpdated?: string }>
): BrandMemoryEntry[] {
  const now = nowIso();
...
export function saveApiKey(id: string, rawKey: string): ApiKeyEntry {
...
export function testConnection(id: string): ConnectionTestResult {
  const rec = assertFound(getState().apiKeys.find((k) => k.id === id), `API key ${id}`);
...
export interface SettingsPatch {
  schedule?: Partial<DashboardSettings['schedule']>;
  notifications?: Partial<DashboardSettings['notifications']>;
  system?: Partial<DashboardSettings['system']>;
  agentToggles?: AgentToggle[];
}

export function saveSettings(patch: SettingsPatch): DashboardSettings {
